...
class CloudFunctionsGenerator:
    ...
    def generate(self, context: GeneratorContext) -> GeneratedProject:
        ...
        trigger_type = str(values.get("trigger_type", "HTTP")).upper()
        if trigger_type not in _TRIGGER_TYPES:
            raise ValueError(
                f"Unsupported trigger_type value: {trigger_type}. "
                "Must be HTTP or PUBSUB."
            )

        pubsub_trigger_topic = str(
            values.get("pubsub_trigger_topic", "")
        ).strip()
        if trigger_type == "PUBSUB" and not pubsub_trigger_topic:
            raise ValueError(
                "pubsub_trigger_topic is required when trigger_type is "
                "PUBSUB (expected format: "
                "projects/<project>/topics/<topic>)."
            )

        pubsub_trigger_topic_default = (
            f'"{pubsub_trigger_topic}"' if pubsub_trigger_topic else "null"
        )
        ...
        template_values = {
            ...
            "pubsub_trigger_topic_default": pubsub_trigger_topic_default,
